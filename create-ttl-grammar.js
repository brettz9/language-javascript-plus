// File obtained from https://github.com/gandm/language-babel/blob/master/src/create-ttl-grammar.js
// License MIT: https://github.com/gandm/language-babel/blob/master/LICENSE.md
...
class CreateTtlGrammar {
  ...
  TTL_GRAMMAR_NAME = 'language-javascript-plus-extension';
  TTL_SCOPENAME = `languagejavascriptplus.ttlextension`;

  constructor(observeConfig = false) {
    if (observeConfig)   {
      // look for changes in tagged template handlers
      this.disposable.add(atom.config.observe('language-javascript-plus.taggedTemplateGrammar', this.observeTtlConfig.bind(this, 10000)));
    }
  }
      ...
      this.doesGrammarFileExist(ttlFilename)
        .then((ifFileExists) => {
          if (ifFileExists) {
            ...
            .then(() => {
              atom.notifications.addInfo('language-javascript-plus', {detail: `Grammar created at \n${ttlFilenameAbsolute}`,dismissable: true});
              resolve(ttlFilename);
            })
            .catch((err) => {
              atom.notifications.addWarning('language-javascript-plus', {detail: `${err.message}`,dismissable: true});
              reject(err);
            });
          }
        });
  ...
  "comment": "Auto generated Tag Extensions for language-javascript-plus",
  ...
  // get full path to the language-javascript-plus grammar file dir
  getGrammarPath() {
    return path.normalize(
      path.resolve(atom.packages.loadedPackages['language-javascript-plus'].path, './grammars')
    );
  }

  // get an array of all language-javascript-plus grammar files
  getGrammarFiles() {
    return new Promise((resolve,reject) => {
  ...
  getTtlConfig() {
    return atom.config.get('language-javascript-plus').taggedTemplateGrammar;
  }
